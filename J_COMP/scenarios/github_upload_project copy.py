...
logging.basicConfig(filename=r"D:\Software-testing-jcomp\J_COMP\logs\app.log", format='%(filename)s %(asctime)s %(message)s', filemode='a')
...
try:
    ...
    code.click()
    logger.info("Successfully clicked on code")
    time.sleep(1)
    ...
    if download:
        ...
        logger.error("Element not found")
    download.click()
    logger.info("Successfully clicked on download")
    # END: ed8c6549bwf9
...
try:
    driver.get('https://github.com/Vidoosh/Image-colorizer')
    time.sleep(2)
    code = driver.find_element(By.CSS_SELECTOR, '#repo-content-pjax-container > div > div > div.Layout.Layout--flowRow-until-md.Layout--sidebarPosition-end.Layout--sidebarPosition-flowRow-end > div.Layout-main > div.file-navigation.mb-3.d-flex.flex-items-start > span.d-none.d-md-flex.ml-2 > get-repo > details > summary')
    if code:
        pass
    else:
        time.sleep(2)
    ...
    logger.info("Successfully clicked on code")
    time.sleep(1)
    download = driver.find_element(By.CSS_SELECTOR, '#local-panel > ul > li:nth-child(3)')
    if download:
        pass
    else:
        time.sleep(2)
